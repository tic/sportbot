...
import { dateObjectToMMDDYYYY, dateObjectToYYYYMMDD } from '../services/util.service';
...
const collect = async () => {
  try {
    const url = config.source.ncaaf.url.replace(
      '<$YYYYMMDD>',
    ...
    logError(LogCategoriesEnum.SCRAPE_FAILURE, config.source.ncaaf.identifier, String(error));
    return [];
  }
};
...
const mergeToDb = async (events: EventType[]) => {
  try {
    const result = await collections.ncaaf.bulkWrite(events.map((event) => ({
      updateOne: {
    ...
    logError(LogCategoriesEnum.DB_MERGE_FAILURE, config.source.ncaaf.identifier, String(error));
    return false;
  }
};
...
const announcer = async () => {
  try {
    ...
    const events = (await collections.ncaaf.find({
      startDay,
      $or: config.source.ncaaf.followedTeams.map((team) => [
    ...
    return {
      events: events.map((event) => ({
        ...event,
        title: `(NCAA Football) ${event.title}`,
      })),
    };
  } catch (error) {
    logError(LogCategoriesEnum.ANNOUNCE_FAILURE, config.source.ncaaf.identifier, String(error));
    return {
      events: [],
    };
  }
